...
with open('data/runandbun_move_changes.json') as f:
    runandbun_move_changes = json.load(f)
...
RUNANDBUN_DEF_HALVING_MOVES = {'explosion', 'selfdestruct', 'mistyexplosion'}


def _parse_runandbun_numeric(value):
...
def _parse_runandbun_type(value):
    if value in (None, 'None'):
        return None
    return value.split('>')[-1].strip()


def _apply_note_text(move, note):
    note = note or ''
    if 'No longer steal items' in note:
        move['desc'] = "Inflicts damage without stealing the target's item."
        move['short_desc'] = "Deals damage without stealing items."
    elif 'Usable by any Pok' in note:
        move['desc'] = 'Causes all adjacent foes to fall asleep.'
        move['short_desc'] = 'Puts adjacent foes to sleep.'
    elif 'Functions like Protect' in note:
        move['priority'] = 4
        if move['primary'].get('self') is None:
            move['primary']['self'] = {'boosts': None, 'status': None, 'volatile_status': None}
        move['primary']['self']['boosts'] = None
        move['primary']['self']['volatile_status'] = 'protect'
        move['flags']['protect'] = True
        move['flags']['snatch'] = False
        move['desc'] = 'Protects the user from most attacks for one turn. Repeated use is less likely to succeed.'
        move['short_desc'] = 'Protects the user this turn.'
    elif 'Can be used multiple times without fail' in note:
        move['desc'] = 'If the user faints this turn, the target also faints.'
        move['short_desc'] = 'If the user faints, the target does as well.'
    elif "Halves target's defense" in note:
        move['desc'] = "Halves the target's defensive stat before dealing damage."
        move['short_desc'] = "Halves the target's defensive stat first."
    elif 'Type determined by the Pok' in note:
        move['desc'] = "This move's type depends on the user's IVs and always has 60 base power."
        move['short_desc'] = 'Type varies based on IVs (60 BP).'
    elif 'Electric-types cannot miss' in note:
        move['desc'] = 'Paralyzes the target. Electric-type users cannot miss with this move.'
        move['short_desc'] = 'Paralyzes; Electric-type users cannot miss.'


def _apply_runandbun_move_changes(moves_data, changes):
    name_to_id = {data['name'].lower(): move_id for move_id, data in moves_data.items()}
    alias_map = {
    ...
    for row in changes:
        move_id = resolve_id(row.get('move'))
        if move_id is None:
            missing.add(row.get('move'))
        else:
            move = moves_data[move_id]
            bp = _parse_runandbun_numeric(row.get('bp'))
            if bp is not None:
                move['base_power'] = bp
            pp = _parse_runandbun_numeric(row.get('pp'))
            if pp is not None:
                move['pp'] = pp
            accuracy = _parse_runandbun_numeric(row.get('accuracy'))
            if accuracy is not None:
                move['accuracy'] = accuracy
            effect = _parse_runandbun_numeric(row.get('effect_chance'))
            if effect is not None and move['secondary']:
                move['secondary'][0]['chance'] = effect
            new_type = _parse_runandbun_type(row.get('type'))
            if new_type is not None:
                move['type'] = new_type

        note_move = row.get('note_move')
        note_text = row.get('note')
        if note_move not in (None, 'None') and note_text:
            note_lookup.setdefault(note_move, set()).add(note_text)

    for note_move_name, note_texts in note_lookup.items():
        target_id = resolve_id(note_move_name)
        if target_id is None:
            missing.add(note_move_name)
            continue
        move = moves_data[target_id]
        for note in note_texts:
            _apply_note_text(move, note)

    missing.discard(None)
    return sorted(missing)


RUNANDBUN_MISSING_MOVE_OVERRIDES = _apply_runandbun_move_changes(moves_raw_data, runandbun_move_changes)
