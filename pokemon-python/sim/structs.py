...
class Pokemon:
    ...
    battle : 'Battle' = field(default=None, repr=False)
    ...
    substitute_hp : int = 0
    stockpile : int = 0
    disguise_broken : bool = False
    ivs : Tuple[int, int, int, int, int, int] = field(init=False)
    hidden_power_type : str = field(init=False)
    ...
    def __post_init__(self, poke, packed):
        ...
        self.ivs = poke.ivs
        self.hidden_power_type = calculate_hidden_power_type(self.ivs)
...
class Battle:
    ...
    def set_up(self):
        ...
        for pokemon in get_active_pokemon(self):
            apply_entry_effects(self, pokemon)
...
def calculate_stats(P:Pokemon, evs:List[int], ivs:List[int]) -> Stats:
    ...
    lvl = P.level
    iv = ivs[0] #hp iv
    ev = evs[0] if EVS_ENABLED else 0
    ...
    for stat in stats:
        ...
        iv = ivs[i]
        ev = evs[i] if EVS_ENABLED else 0
...
def calculate_hidden_power_type(ivs: Any) -> str:
    if ivs is None:
        return 'Normal'
    if isinstance(ivs, dict):
        order = ['hp', 'atk', 'def', 'spa', 'spd', 'spe']
        values = [int(ivs.get(stat, 31)) for stat in order]
    else:
    ...
    weight = bits[0] + (bits[1] << 1) + (bits[2] << 2) + (bits[3] << 3) + (bits[4] << 4) + (bits[5] << 5)
    index = math.floor(weight * 15 / 63)
    return HIDDEN_POWER_TYPES[index]


WEATHER_ABILITIES = {
    'drought': 'sunlight',
    'desolateland': 'sunlight',
    'drizzle': 'rain',
    'primordialsea': 'rain',
...
def prepare_pokemon_for_entry(pokemon:Pokemon) -> None:
    if pokemon is None:
        return
    if pokemon.status == 'slp':
        pokemon.sleep_n = random.randint(1, 3)


def apply_entry_effects(B:Battle, pokemon:Pokemon) -> None:
    if pokemon is None:
        return
    prepare_pokemon_for_entry(pokemon)
    ability = pokemon.ability
    weather = WEATHER_ABILITIES.get(ability)
    if weather:
        B.weather = weather
        B.weather_permanent = True
        B.weather_n = None
    terrain = TERRAIN_ABILITIES.get(ability)
    if terrain:
        B.terrain = terrain
        B.terrain_permanent = True
