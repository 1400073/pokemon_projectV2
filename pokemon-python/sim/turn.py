...
def priority_bonus(user:Pokemon, move:dex.Move) -> int:
    bonus = 0
    if move is None:
        return 0
    if user.ability == 'galewings' and move.type == 'Flying':
        bonus += 1
    return bonus
...
def turn_end(B:Battle) -> None:
    ...
    if B.weather in ['sunlight', 'rain', 'sandstorm', 'hail'] and not B.weather_permanent:
        if B.weather_n is not None:
            B.weather_n -= 1
            if B.weather_n <= 0:
                B.weather = 'clear'
                B.weather_n = 0
                B.weather_permanent = False
...
def run_action(B, a : Action) -> None:
    ...
    if a.target == 'all': 
        move = a.move._replace(base_power = a.move.base_power * 0.75)
        for pokemon in get_active_pokemon(B):
            run_move(B, a.user, move, pokemon)
        return
...
def calc_damage(B:Battle, user:Pokemon, move:dex.Move, target:Pokemon) -> int:
    ...
    if move.id in getattr(dex, 'RUNANDBUN_DEF_HALVING_MOVES', set()):
    ...
    boost_type = terrain_boosts.get(B.terrain)
...
def accuracy_check(B:Battle, user:Pokemon, move:dex.Move, target:Pokemon) -> bool:
    ...
    if move.id == 'blizzard' and B.weather == 'hail':
        return True
    if move.id == 'thunderwave' and 'Electric' in user.types:
        return True
...
def unique_moves_after_damage(B:Battle, user:Pokemon, move:dex.Move, target:Pokemon, dmg:int):
    ...
    if move.weather is not None and B.weather != move.weather:
        ...
        if user.item == 'icyrock' and B.weather == 'hail':
            B.weather_n = 8
    if move.id == 'steelroller' and B.terrain != '':
        B.terrain = ''
        B.terrain_permanent = False
...
def create_move(B:Battle, p:Pokemon, c:Decision) -> dex.Move:
    ...
    if move.id.startswith('hiddenpower'):
        hp_type = getattr(p, 'hidden_power_type', 'Normal') or 'Normal'
        move = move._replace(type=hp_type)
        move = move._replace(base_power=60)
...
def resolve_priority(action, B:Battle, T:Player) -> float:
    ...
    if action.action_type == 'switch':
        ...
        effective_priority = action.move.priority + priority_bonus(action.user, action.move)
        action_priority_tier = 3 + (5 - effective_priority)
